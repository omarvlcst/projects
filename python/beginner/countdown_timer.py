...
clockWindow.configure(background="gray")
...
# Set strings to default value before initializing
hourString.set("00")
minuteString.set("00")
secondString.set("00")
...
def runTimer():
    ...
    while clockTime > -1:
        #pass in two numbers: number, remainder
        ...
        # Let the user know if the timer has expired its duration
        if clockWindow == 0:
            messagebox.showinfo("", "Time has expired ! Restart the timer or exit.")

        clockTime -= 1

setTimeButton = Button(clockWindow, text="Set your time start", bd="6", command=runTimer)
setTimeButton.place(relx=0.5, rely=0.5, anchor=CENTER)

### Keep looping the timer
